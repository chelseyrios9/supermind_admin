...
const AddActionSuperpowersTab = ({ values, setFieldValue, errors, updateId }) => {
  ...
  const { data: superpowers, isLoading: superpowerloading } = useQuery([ActionSuperpowerAPI], () => request({ url: ActionSuperpowerAPI, method: 'get' }), { refetchOnWindowFocus: false, select: (res) => res?.data });
