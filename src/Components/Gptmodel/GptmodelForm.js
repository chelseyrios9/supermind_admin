  ...
  return (
    <Formik
      ...
      initialValues={{
        ...
        token_price: updateId ? oldData?.token_price : "",
        is_public: updateId ? Boolean(Number(oldData?.is_public)) : true,
        status: updateId ? Boolean(Number(oldData?.status)) : true,
      }}
      validationSchema={YupObject({ name: nameSchema, description: descriptionSchema, api_url: nameSchema, api_key: nameSchema, token_price: nameSchema })}
